...
## 仓配服务商品整理图库图片替换（2026-08-21，本地完成）
...
- 本地桌面端和 `390×844` 移动端实测通过：两张新图加载完成、4张缩略图数量不变、移动端无横向溢出；隔离环境 `CI=1 npm run verify:release` 完整通过（366个 Astro/TypeScript 文件0诊断、45个 Vitest 文件292项通过、Playwright 37项通过且7项按配置跳过、正式域名契约3项通过、两轮生产构建成功）。当前尚未提交、推送或部署。
