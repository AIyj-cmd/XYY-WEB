...
健康检查已发布到 `wz.tomatopia.top` 的 `20260814T093101Z` release，应用提交为 `ab3b45d`。
...
5. `/healthz` 除 Directus ping 外会读取两枚运行令牌的权限映射，检查18个公开内容集合是否
   具备已发布内容读取权限，并确认独立联系令牌具备 `contact_leads` 创建权限；单次健康检查
   共3个Directus请求，不读取咨询记录。完整逐集合读取由发布验收命令执行。
